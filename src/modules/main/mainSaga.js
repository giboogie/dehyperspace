...
async function getMainDataApi(jwt)
{
    ...
    try{
        ...
        console.log('총에러'+err.response)
        throw err;
    }
}

function* mainDataFlow () {
    let response;
    let token;
    try {
        token = yield AsyncStorage.getItem('token')
        response = yield call (getMainDataApi, token);
        yield put(mainDataSuccess(response));
    }catch (error) {
        // error? send it to redux
        // yield put(errorIn(error));
        console.log(error.response.data)
        yield put(mainDataError(error));
...
function* mainDataWatcher () { 
    ...
    while( true ){
        ...
        // forward...
        const { payload } = yield take(MAINDATA_REQUESTING);
        ...
        const task = yield fork(mainDataFlow);
...
export default mainDataWatcher;
