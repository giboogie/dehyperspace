...
async function findIDApi(email)
{
...
function* findIDFlow (email) {
    let response;
    try {
        response = yield call (findIDApi,email);
        yield put(findIDResult(response));
    }catch (error) {
...
function* findIDWatcher () { 
    ...
    while( true ){
        ...
        // forward...
        const { payload } = yield take(FIND_ID_REQUESTING);
        ...
        const task = yield fork(findIDFlow, email);
